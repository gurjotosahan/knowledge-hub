...
interface SidebarProps {
  ...
  onSetAppMode?: (mode: "knowledge" | "clientResearch") => void;
  isOpen?: boolean;
  onClose?: () => void;
}
...
export default function Sidebar({
  ...
  return (
    ...
    >
      ...
      {/* ── Tools ── */}
      <div className="px-3 py-2 border-b border-slate-700 shrink-0 space-y-1">
        <Link
          href="/slide-composer"
        ...
        <button
          onClick={() => { onSetAppMode?.("clientResearch"); onClose?.(); }}
          className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium text-slate-300 hover:bg-slate-800 hover:text-white transition-colors"
        >
          <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
        ...
        <button
          onClick={() => { router.push("/rfp-analyzer"); onClose?.(); }}
          className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium text-slate-300 hover:bg-slate-800 hover:text-white transition-colors"
        >
          <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
